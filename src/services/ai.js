...
// Generate dynamic follow-up questions using Gemini (free tier)
...
// Generate the full research report using DeepSeek
...

{
  ...
  "key_insights": [
    { "insight": "string", "evidence": "string", "source_url": "string" }
  ],
  
  "top_quotes": [
    { "quote": "string", "subreddit": "string", "upvotes": number, "url": "string" }
  ],
  
  "subreddits": [
    { "name": "string", "relevance": "string", "url": "string" }
  ],
  ...
  "competitors": [
    { "name": "string", "url": "string", "what_people_say": "string", "gaps": ["string"] }
  ],
  ...
  "reddit_threads": [
    { "title": "string", "url": "string", "why_relevant": "string", "upvotes": number }
  ],
...
// Generate social media posts using DeepSeek (Launch Voice feature)
const generatePosts = async (reportData, query) => {
  const prompt = `You are a social media content expert helping a founder share their market research.

RESEARCH TOPIC: ${query}
VERDICT: ${reportData.verdict}
KEY INSIGHTS: ${JSON.stringify(reportData.key_insights?.slice(0, 3))}
TOP QUOTES: ${JSON.stringify(reportData.top_quotes?.slice(0, 2))}
DEMAND SCORE: ${reportData.demand_score}/100

Generate 4 social media posts in this exact JSON format (return ONLY valid JSON):
{
  "twitter_thread": {
    "platform": "twitter",
    "tone": "storytelling",
    "content": "Full Twitter/X thread with tweet numbers (1/, 2/, etc.) using real data from the research"
  },
  "linkedin": {
    "platform": "linkedin", 
    "tone": "data_driven",
    "content": "Professional LinkedIn post with data points and insights"
  },
  "reddit": {
    "platform": "reddit",
    "tone": "humble",
    "content": "Authentic Reddit-style post that doesn't feel promotional, shares genuine findings"
  },
  "hook": {
    "platform": "hook",
    "tone": "hype",
    "content": "Just one powerful hook sentence to grab attention"
  }
}`;
...
// Ask the Data - chat with report using DeepSeek
const askReport = async (question, reportData) => {
  const prompt = `You are an AI analyst. You have access to a market research report. Answer the user's question using ONLY the data in the report. Be concise and specific.

REPORT DATA: ${JSON.stringify(reportData)}

USER QUESTION: ${question}

Answer in 2-4 sentences maximum. Be direct and cite specific data from the report.`;
